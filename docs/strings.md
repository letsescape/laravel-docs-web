...
`__` 함수는 주어진 번역 문자열 또는 번역 키를 [언어 파일](/docs/localization)을 이용해 번역합니다.
...
값의 배열을 전달하면, 해당 배열 중 하나라도 포함되어 있는지 확인할 수 있습니다.
...
`isUrl` 메서드는 다양한 프로토콜을 유효한 것으로 간주합니다. 특정 프로토콜만 허용하고 싶다면 해당 프로토콜 배열을 두 번째 인수로 전달할 수 있습니다.
...
`Str::lcfirst` 메서드는 주어진 문자열의 첫 글자를 소문자로 반환합니다.
...
`Str::plural` 메서드는 단수형 문자열을 복수형으로 변환합니다. 이 함수는 [라라벨의 복수화 기능을 지원하는 모든 언어](/docs/localization#pluralization-language)를 지원합니다.
...
`Str::pluralStudly` 메서드는 StudlyCaps 형태의 단어 문자열을 복수형으로 변환합니다. 이 함수 역시 [라라벨의 복수화 기능을 지원하는 모든 언어](/docs/localization#pluralization-language)를 지원합니다.
...
`replace` 메서드는 `caseSensitive` 인자도 받을 수 있습니다. 기본적으로 이 메서드는 대소문자를 구분(case sensitive)합니다.
...
`replaceMatches` 메서드는 또한 클로저(익명 함수)를 전달할 수 있습니다. 이 경우, 해당 패턴과 일치하는 문자열마다 클로저가 호출되어, 그 결과로 대체할 값을 반환할 수 있습니다.
...
`Str::singular` 메서드는 복수형 문자열을 단수형으로 변환합니다. 이 함수는 [라라벨의 복수화 기능이 지원하는 모든 언어](/docs/localization#pluralization-language)에서 동작합니다.
...
`trans` 함수는 주어진 번역 키를 사용해 [언어 파일](/docs/localization)에서 번역된 문자열을 반환합니다.

```php
echo trans('messages.welcome');
```

지정한 번역 키가 존재하지 않을 경우, `trans` 함수는 해당 키 자체를 반환합니다. 따라서 위 예시에서, 번역 키가 존재하지 않으면 `trans` 함수는 `messages.welcome`을 반환하게 됩니다.
...
배열을 전달할 수도 있습니다. 문자열이 배열의 값 중 하나로 끝난다면, 해당 값을 한 번만 제거합니다.
...
`kebab` 메서드는 지정한 문자열을 `kebab-case` 형태로 변환합니다.
...
`plural` 메서드는 단수 명사 문자열을 복수 형태로 변환합니다. 이 기능은 [라라벨의 복수화가 지원하는 모든 언어](/docs/localization#pluralization-language)를 지원합니다.
...
`singular` 메서드는 문자열을 단수형으로 변환합니다. 이 함수는 [라라벨의 복수화 지원 언어](/docs/localization#pluralization-language)를 모두 지원합니다:
...
`substr` 메서드는 주어진 시작 위치와 길이만큼 문자열의 일부를 반환합니다:
...
`toHtmlString` 메서드는 주어진 문자열을 `Illuminate\Support\HtmlString` 인스턴스로 변환하며, Blade 템플릿에서 렌더링할 때 이 값은 이스케이프되지 않습니다:
...
`transliterate` 메서드는 주어진 문자열을 가능한 한 가까운 ASCII 표현으로 변환하려고 시도합니다:
...
`trim` 메서드는 문자열의 앞뒤 공백을 제거합니다. PHP의 기본 `trim` 함수와 달리, 라라벨의 `trim` 메서드는 유니코드 공백 문자도 함께 제거합니다:
...
`ltrim` 메서드는 문자열의 왼쪽(앞부분) 공백을 제거합니다. PHP의 기본 `ltrim` 함수와 달리, 라라벨의 `ltrim` 메서드는 유니코드 공백 문자도 함께 제거합니다:
...
`rtrim` 메서드는 문자열의 오른쪽(끝부분) 공백을 제거합니다. PHP의 기본 `rtrim` 함수와 달리, 라라벨의 `rtrim` 메서드는 유니코드 공백 문자도 함께 제거합니다:
...
`ucsplit` 메서드는 문자열을 대문자 기준으로 나눠 컬렉션으로 분할합니다:
...
`unwrap` 메서드는 지정한 문자열로 감싸진 시작과 끝 부분을 제거합니다:
...
`when` 메서드는 지정한 조건이 `true`일 때 주어진 클로저를 실행합니다. 이때 플루언트 문자열(문자열 객체) 인스턴스가 클로저로 전달됩니다:
...
필요하다면, `when` 메서드의 세 번째 인자로 클로저를 하나 더 전달할 수 있습니다. 이때 조건이 `false`로 평가되면 이 클로저가 실행됩니다.

<a name="method-fluent-str-when-contains"></a>
#### `whenContains`

`whenContains` 메서드는 문자열이 지정한 값을 포함하고 있을 때 주어진 클로저를 실행합니다. 이 클로저에는 플루언트 문자열 인스턴스가 전달됩니다:
...
필요하다면, 세 번째 인자로 또 다른 클로저를 전달할 수 있습니다. 이때 문자열에 값이 포함되지 않았다면 해당 클로저가 실행됩니다.

또한 배열 형태로 복수의 값들을 전달하여, 문자열에 해당 배열의 값 중 하나라도 포함되어 있는지 확인할 수 있습니다:
...
`whenContainsAll` 메서드는 문자열에 전달한 모든 하위 문자열이 포함되어 있을 때만 지정한 클로저를 실행합니다. 이때 플루언트 문자열 인스턴스가 클로저로 전달됩니다:
...
`whenEmpty` 메서드는 문자열이 빈 값일 때 주어진 클로저를 실행합니다. 클로저에서 값을 반환하면, 그 값이 `whenEmpty` 메서드의 반환값이 됩니다. 클로저에서 값을 반환하지 않으면, 플루언트 문자열 인스턴스가 반환됩니다:
...
`whenNotEmpty` 메서드는 문자열이 비어 있지 않을 때 주어진 클로저를 실행합니다. 클로저가 값을 반환하면, 그 값이 `whenNotEmpty` 메서드의 반환값이 됩니다. 클로저가 값을 반환하지 않을 경우에는 유연한 문자열 인스턴스(fluent string instance)가 반환됩니다.
...
`whenStartsWith` 메서드는 문자열이 지정한 하위 문자열로 시작할 경우, 주어진 클로저를 실행합니다. 이때 클로저에는 유연한 문자열 인스턴스가 전달됩니다.
...
`whenEndsWith` 메서드는 문자열이 지정한 하위 문자열로 끝나는 경우, 주어진 클로저를 실행합니다. 클로저에는 유연한 문자열 인스턴스가 전달됩니다.
...
`whenExactly` 메서드는 문자열이 지정한 문자열과 완전히 일치할 때, 주어진 클로저를 실행합니다. 클로저에는 유연한 문자열 인스턴스가 전달됩니다.
...
`whenNotExactly` 메서드는 문자열이 지정한 문자열과 완전히 일치하지 않을 때, 주어진 클로저를 실행합니다. 이때 클로저에는 유연한 문자열 인스턴스가 전달됩니다.
...
`whenIs` 메서드는 문자열이 지정한 패턴과 일치할 때 클로저를 실행합니다. 패턴에 별표(*)를 와일드카드로 사용할 수 있습니다. 클로저에는 유연한 문자열 인스턴스가 전달됩니다.
...
`whenIsAscii` 메서드는 문자열이 7비트 ASCII인 경우에만 클로저를 실행합니다. 클로저에는 유연한 문자열 인스턴스가 전달됩니다.
...
`whenIsUlid` 메서드는 문자열이 유효한 ULID인 경우에만 클로저를 실행합니다. 클로저에는 유연한 문자열 인스턴스가 전달됩니다.
...
`whenIsUuid` 메서드는 문자열이 유효한 UUID인 경우에만 클로저를 실행합니다. 클로저에는 유연한 문자열 인스턴스가 전달됩니다.
...
`whenTest` 메서드는 문자열이 지정한 정규 표현식과 일치할 때, 주어진 클로저를 실행합니다. 클로저에는 유연한 문자열 인스턴스가 전달됩니다.
...
`wordCount` 메서드는 해당 문자열이 포함하는 단어의 개수를 반환합니다.
...
`words` 메서드는 문자열의 단어 수를 제한할 수 있습니다. 필요하다면, 잘린 문자열 뒤에 추가로 붙일 문자열을 지정할 수 있습니다.
...
`wrap` 메서드는 지정한 문자열(또는 문자열의 쌍)로 원본 문자열을 감쌉니다.
